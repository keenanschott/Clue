...
/**
 * BoardAdjTargetTest
 * A series of tests to examine the movement algorithm and adjacency list
 * creation algorithm as it pertains to the initialization of the game board.
 * DATE: 3/26/2023
 * 
...
public class BoardAdjTargetTest {
	...
	public void testTargetsOccupied() {
		// (21,7) - make sure player can't move through a door occupied by another
		// player
		...
		targets = board.getTargets();
		assertEquals(4, targets.size());
		assertTrue(targets.contains(board.getCell(3, 24))); // room centers, even if occupied, can still be moved to per
															// the rules
		assertTrue(targets.contains(board.getCell(4, 19)));
		assertTrue(targets.contains(board.getCell(6, 19)));
		assertTrue(targets.contains(board.getCell(5, 18)));
